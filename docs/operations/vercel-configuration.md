...
| `GOOGLE_TTS_CREDENTIALS_JSON` | service-account JSON (one line) | **KC-019** Digital Rafeeq TTS/STT (server-only). Must **not** be the only credential for Auth claim provisioning. |
| `FIREBASE_SERVICE_ACCOUNT_JSON` | firebase-adminsdk JSON for `karkun-connect-75c68` | **Required (KC-0100.5).** `POST /api/rukn-claims-provision` verifies ID tokens against this project. Wrong/missing SA caused production `401 Invalid or expired ID token`. |
| `FIREBASE_PROJECT_ID` | `karkun-connect-75c68` | **Required (KC-0100.5).** Must match the service account `project_id`. |
