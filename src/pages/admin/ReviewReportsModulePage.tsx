  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      <section className="rounded-(--radius-card) border border-border bg-surface p-6 shadow-card">
        ...
        <p className="mt-1 text-sm text-secondary">
          Current week attendance status for each Karkun.
        </p>
        <div className="mt-4 overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border text-secondary">
                ...
                <th className="px-3 py-2 font-medium">Attendance Status</th>
                <th className="px-3 py-2 font-medium">Remarks</th>
              </tr>
            </thead>
            <tbody>
              {ijtemaSummaries.map((summary) => (
                <tr key={summary.karkunId} className="border-b border-border/60">
                  <td className="px-3 py-3">
                    <Link
                      to={adminKarkunProfilePath(summary.karkunId)}
                  ...
                  <td className="px-3 py-3 text-text-heading">
                    Week ending {summary.weekLabel}
                  </td>
                  <td className="px-3 py-3 text-text-heading">{summary.status}</td>
                  <td className="px-3 py-3 text-text-heading">{summary.remarks ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
