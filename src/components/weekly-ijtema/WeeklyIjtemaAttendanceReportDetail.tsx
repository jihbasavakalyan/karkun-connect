/**
 * Weekly Ijtema attendance report body — same getWeeklyIjtemaReport source as the
 * Admin report page. Presentation only; opening this view is read-only.
 */

import { useEffect, useMemo, useState } from 'react'
import { Link } from 'react-router-dom'
import { adminWeeklyIjtemaReportPath } from '@/constants/routes'
import {
...
export function WeeklyIjtemaAttendanceReportDetail({
  eventId,
  variant,
  onSelectEventId,
}: {
  ...
  if (!report) {
    return (
      <div className={variant === 'modal' ? 'flex min-h-0 flex-1 flex-col' : ''}>
        ...
        <HistoricalWeekLinks
          ...
          onSelectEventId={variant === 'modal' ? onSelectEventId : undefined}
        />
      </div>
    )
  }
  ...
  const deadlineLabel = new Date(event.submissionDeadline).toLocaleString('en-GB', {
    day: 'numeric',
    month: 'short',
    hour: '2-digit',
    minute: '2-digit',
  ...
  const summary = (
    <div className="shrink-0 space-y-3">
      <div>
        <h3 className="text-lg font-semibold text-text-heading">{event.title}</h3>
        <p className="mt-0.5 text-sm text-secondary">
          {formatWeeklyIjtemaMeetingLabel(event.meetingDate)} · Audience:{' '}
          {formatWeeklyIjtemaAudienceLabel(event)} · {event.status}
          {event.openedAutomatically ? ' · Auto window' : ''}
        </p>
        <p className="mt-0.5 text-xs text-secondary">Deadline {deadlineLabel}</p>
      </div>
      ...
      <section className="grid grid-cols-2 gap-2 lg:grid-cols-4 xl:grid-cols-5">
        <MetricCard label="Meeting Date" value={formatWeeklyIjtemaMeetingLabel(event.meetingDate)} />
        <MetricCard label="Attendance · Present" value={String(report.present)} />
  ...
  const detail = (
    <div
      className={
        variant === 'modal'
          ? 'mt-3 min-h-0 sm:flex-1 sm:overflow-y-auto sm:overscroll-contain'
          : 'mt-4'
      }
    >
          ...
          <div className="mt-2 overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="border-b border-border text-xs uppercase tracking-wide text-secondary">
                <tr>
                  <th className="px-2 py-1.5 font-semibold">Rukn</th>
                  <th className="px-2 py-1.5 font-semibold">Rukn attendance</th>
                  <th className="px-2 py-1.5 font-semibold">Connected</th>
                  <th className="px-2 py-1.5 font-semibold">Present</th>
                  <th className="px-2 py-1.5 font-semibold">Absent</th>
              ...
              <tbody>
                {report.ruknRows.map((row) => (
                  <tr key={row.ruknId} className="border-b border-border/70">
                    <td className="px-2 py-1.5 font-medium text-text-heading">{row.ruknName}</td>
                    <td className="px-2 py-1.5">{row.ruknAttendance}</td>
                    <td className="px-2 py-1.5">{row.assigned}</td>
                    <td className="px-2 py-1.5">{row.present}</td>
                    <td className="px-2 py-1.5">{row.absent}</td>
                    <td className="px-2 py-1.5">{row.submitted ? `${row.attendancePct}%` : '—'}</td>
  ...
  if (variant === 'modal') {
    return (
      <div className="flex min-h-0 flex-1 flex-col overflow-y-auto overscroll-contain sm:overflow-hidden">
        {summary}
        {detail}
      </div>
    )
  ...
  return (
    <ul className="mt-2 flex flex-wrap gap-2">
      {sorted.map((item, index) => {
        ...
        const isPrevious =
          !isCurrent && (currentIndex >= 0 ? index === currentIndex + 1 : index === 0)
        const scopeLabel = isCurrent ? 'Current Week' : isPrevious ? 'Previous Week' : 'Specific Week'
        const className = [
        ...
        return (
          <li key={item.id}>
            {onSelectEventId ? (
              <button type="button" className={className} onClick={() => onSelectEventId(item.id)}>
                <span>{formatWeeklyIjtemaMeetingLabel(item.meetingDate)}</span>
                <span className="font-normal opacity-80">{scopeLabel}</span>
              </button>
            ) : (
              <Link to={adminWeeklyIjtemaReportPath(item.id)} className={className}>
                <span>{formatWeeklyIjtemaMeetingLabel(item.meetingDate)}</span>
                <span className="font-normal opacity-80">{scopeLabel}</span>
              </Link>
            )}
...
function MetricCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-lg border border-border bg-surface px-3 py-2 shadow-card">
      ...
      <p className="mt-0.5 text-base font-semibold text-text-heading sm:text-lg">{value}</p>
    </div>
  )
}
