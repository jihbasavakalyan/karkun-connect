...
export function BulkActionsBar({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-(--radius-card) border border-primary/20 bg-primary/5 p-3">
      ...
      {onMarkIjtemaPresent && (
        <SecondaryButton type="button" className="px-3 py-2 text-sm" onClick={onMarkIjtemaPresent}>
          Mark Ijtema Present
        </SecondaryButton>
      )}
      {onMarkIjtemaAbsent && (
        <SecondaryButton type="button" className="px-3 py-2 text-sm" onClick={onMarkIjtemaAbsent}>
          Mark Ijtema Absent
        </SecondaryButton>
      )}
      {onMarkIjtemaInformed && (
