...
// Final composition: Meeqati+توجہ طلب composed → departments → ijtema → quick → important → campaign
assert(meqatiIdx > 0, 'Meqati year summary rendered')
assert(attentionIdx > meqatiIdx, 'Attention composed into Meeqati summary')
assert(attentionIdx < shobahIdx, 'Attention before Shobah (inside Meeqati compose)')
assert(stack.includes('attention={<AttentionCompact'), 'توجہ طلب merged into Meeqati panel prop')
...
assert(css.includes('.orgdash-meqati-compose'), 'Meqati compose layout present')
assert(css.includes('.orgdash-meqati-gold-divider'), 'gold divider present')
assert(css.includes('background: #e8eef5'), 'Meqati light slate surface present')
