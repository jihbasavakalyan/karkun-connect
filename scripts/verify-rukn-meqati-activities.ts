...
console.log('▶ Home wiring — compact navigation into Meeqati destination, no planning clone')
{
  ...
  assertIncludes(meqatiCard, 'MeqatiYearSummary', 'concise current-year Meeqati summary on Home')
  assertIncludes(meqatiCard, 'resolveMeqatiYear', 'same current-year default as Admin')
  assertIncludes(meqatiCard, 'buildOrganisationalSituation', 'Jamaat-wide Meqati from Admin builder')
  assertIncludes(meqatiCard, 'ROUTES.RUKN_MEQATI_MANSOOBA', 'Meeqati Mansooba detail link')
  assertNotIncludes(meqatiCard, 'useMeqatiYearSelection', 'Home has no Meeqati year selector')
  ...
  assertIncludes(home, 'میقاتی منصوبہ — موجودہ سال', 'Home boundary uses current-year Meeqati title')
