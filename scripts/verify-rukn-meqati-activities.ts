...
console.log('▶ Home wiring — compact navigation into Meeqati destination, no planning clone')
{
  ...
  assert.ok(tarbiyatAt >= 0 && tarbiyatAt < orgAt, 'Tarbiyat before Organisational Information')
  assert.ok(orgAt < meqatiAt, 'Organisational Information before Meeqati Mansooba')
  assert.ok(meqatiAt < karkunAt, 'Meeqati Mansooba before Karkun')
  ...
  assertIncludes(orgCard, 'تنظیمی معلومات', 'Organisational Information heading')
  assertIncludes(orgCard, 'ارکان', 'people metric')
  ...
  assertIncludes(meqatiCard, 'yearUrduRange', 'year context belongs to Meeqati Mansooba')
  assertIncludes(meqatiCard, 'سرگرمیاں', 'activity metrics belong to Meeqati Mansooba')
  assertIncludes(meqatiCard, 'پیش رفت', 'progress belongs to Meeqati Mansooba')
